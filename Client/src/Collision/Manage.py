...
def Setup():
    ...
    #Set the Max number of NPCs to be checked per thread
    if len(NPC.Manage.NPC_Objects) >= 10:
        NPC_PER_THREAD_MAX = len(NPC.Manage.NPC_Objects)/2
    else:
        NPC_PER_THREAD_MAX = len(NPC.Manage.NPC_Objects)
    ...
    #Start a new object trigger detection block thread for the remaining objects
    startElement = (overallCount - count) + 1
    endElement = startElement + count
    #print "[Collision Manager]:{Objects}:-Starting new thread: start=", startElement, " end=", endElement
    thread.start_new_thread(Object_Trigger_Detection_Thread, (startElement, endElement))
    ...
    #Debug output
    #print "\n[Collision Manager]:{NPCs}:-Overall length: ", len(Maps.Tiled.Level_Objects)

    #Seperate NPCs into trigger detection threads
    for i in range(0, len(NPC.Manage.NPC_Objects)):
        if count >= NPC_PER_THREAD_MAX:
            #Set the elements
            startElement = i - count
            endElement = startElement + count

            #Debug output
            #print "[Collision Manager]:{NPCs}:-Starting new thread: start=", startElement, " end=", endElement

            #Start a new collision detection block thread
            thread.start_new_thread(NPC_Trigger_Detection_Thread, (startElement, endElement))
    ...
    #Start a new npc trigger detection block thread for the remaining npcs
    startElement = (overallCount - count) + 1
    endElement = startElement + count
    #print "[Collision Manager]:{NPCs}:-Starting new thread: start=", startElement, " end=", endElement
    thread.start_new_thread(NPC_Trigger_Detection_Thread, (startElement, endElement))
...
#NPC Trigger Detection - Main Player and NPCs
def NPC_Trigger_Detection_Thread(start, end):
    while True:
        #While checking npc area collision, ignore any issues (Incase obj has been removed in another thread, etc)
        try:
            #Check Collision Against NPCs
            for i in range(int(start), int(end)):
                if (NPC.Manage.NPC_Objects[i].Sprites[NPC.Manage.NPC_Objects[i].Current_Frame].get_rect(\
                size=(NPC.Manage.NPC_Objects[i].Sprites[NPC.Manage.NPC_Objects[i].Current_Frame].get_width(), \
                      NPC.Manage.NPC_Objects[i].Sprites[NPC.Manage.NPC_Objects[i].Current_Frame].get_height()), \
                center=(NPC.Manage.NPC_Objects[i].Current_X, NPC.Manage.NPC_Objects[i].Current_Y)).colliderect(\
